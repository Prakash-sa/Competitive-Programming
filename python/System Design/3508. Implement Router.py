...
class Router:
    ...
    def forwardPacket(self) -> List[int]:
        try:
            ...
            self.hs.remove(tup)
            self.hm[tup[1]].popleft()
            return tup

    def getCount(self, d: int, startTime: int, endTime: int) -> int:
        return bisect_right(a=self.hm[d],x=endTime)-bisect_left(a=self.hm[d],x=startTime)
